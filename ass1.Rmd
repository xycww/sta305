...
kable(finalTable, digits = 3, caption = "Coefficients and Confidence Intervals for the Possion Model.\n
This table presents the estimated coefficients, standard errors, z-values, p-values, \n and the corresponding 95% confidence intervals (CI) for the model. The coefficient estimates reflect the relationship between each predictor and the number of children, while the confidence intervals provide a range of values within which the true parameter is likely to fall. The exponentiated coefficients (odds ratios) and their confidence intervals indicate the expected multiplicative change in the outcome variable for a one-unit increase in the predictor. Key findings show the significant predictors of fertility, with their associated confidence intervals suggesting the range of possible effects.")
```

Figure 4 presents the means and variances of literacy and age at marriage. In several cases, the variance is at least twice as large as the mean, indicating potential overdispersion. To address this, a Negative Binomial model is applied, which allows the variance to exceed the mean by introducing an additional dispersion parameter. The model is specified as follows:
\[
...
# Display final table with reduced decimal places
kable(finalTable, digits = 3, caption = "Log-rate ratios and 95% confidence intervals for the Negative Binomial model relating school institution type, region, and interactions to the number of violent crimes.")
```
