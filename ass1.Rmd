...
# Summarize categorical variables (ageMarried and literacy)
ageMarried_summary <- portugal %>%
  group_by(ageMarried) %>%
  summarise(Count = n(), Proportion = n() / nrow(portugal)) %>%
  pivot_wider(names_from = ageMarried, values_from = Proportion)  # Pivot to wide format

literacy_summary <- portugal %>%
  group_by(literacy) %>%
  summarise(Count = n(), Proportion = n() / nrow(portugal)) %>%
  pivot_wider(names_from = literacy, values_from = Proportion)  # Pivot to wide format

# Combine all summaries into one table
summary_table <- list(
  "Children" = children_summary,
  "Months Since Married" = monthsSinceM_summary,
  "Age Married (Category)" = ageMarried_summary,
  "Literacy (Yes/No)" = literacy_summary
)
...
```
```{r}
# Example for 'children' variable
hist(portugal$children, main = "Distribution of Children", xlab = "Number of Children", col = "skyblue", breaks = 20)
...
```{r}
dispersion_ratio <- sum(residuals(portugalFit, type="pearson")^2) / df.residual(portugalFit)
dispersion_ratio

```
```{r}
AIC(portugalFit, portugalNB)
